...
pub fn eval_annotation_expr(expr: &DesugaredExpr, env: &Environment) -> FogResult<Annotation> {
    ...
    match expr {
        ...
        DesugaredExpr::FunctionAppl { fn_name, args, .. } if fn_name == "->" && args.len() == 2 => {
            match (
...
pub fn eval_type_annotation_expr(expr: &DesugaredExpr, env: &Environment) -> FogResult<Type> {
    ...
    match expr {
        ...
        DesugaredExpr::FunctionAppl { fn_name, .. } if fn_name == "+" => Err(runtime_error!(
            Some(span),
            "cannot type annotate a value with sum types"
        )),

        DesugaredExpr::FunctionAppl { fn_name, args, .. } if env.contains_type(fn_name) => {
            apply_type_level_function(fn_name, args, env, &span)
        }

        DesugaredExpr::FunctionAppl { .. } => Err(runtime_error!(
            Some(span),
